...
export function TripoLeadView({ task }: TripoLeadViewProps) {
  ...
  // Files View Mode (Grid vs List - Desktop & Mobile)
  ...
  // Starred Entries State (Persisted locally for all users)
  const [starredEntryIds, setStarredEntryIds] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('dm_tripolead_starred_entries');
  ...
  const toggleStarEntry = (entryId: string) => {
    setStarredEntryIds((prev) => {
      ...
      try {
        localStorage.setItem('dm_tripolead_starred_entries', JSON.stringify(next));
      } catch {}
      return next;
    });
  ...
  // Edit Entry Mutation (Admin Only) - Edits hotel_name, district, area, location_link in place
  const editEntryMutation = useMutation({
    mutationFn: (data: { hotel_name: string; district: string; area: string; location_link?: string }) =>
      updateTripoLeadEntry(task.id, editingEntry!.id, data, user?.id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tripoLeadEntries', task.id] });
      queryClient.invalidateQueries({ queryKey: ['tripoLeadRecentEntries', task.id] });
      setEditingModalOpen(false);
      setEditingEntry(null);
      toast.success('TripO Lead entry details updated successfully');
    },
    onError: (err: any) => {
  ...
  // Update Status/Date/Notes Mutation (Admin Only)
  ...
  const handleOpenEditModal = (entry: TripoLeadEntry) => {
    if (!isUserAdmin) {
      toast.error('Access Denied: Only Administrators can edit TripO Lead entries.');
      return;
    }
  ...
  const handleOpenUpdateModal = (entry: TripoLeadEntry) => {
    if (!isUserAdmin) {
      toast.error('Access Denied: Only Administrators can update TripO Lead entries.');
      return;
    }
    ...
    <div className="flex min-h-screen bg-[var(--color-surface-secondary)] relative">
      ...
      <div className={`flex flex-1 flex-col min-w-0 transition-[margin] duration-200 ${sidebarOpen ? 'lg:ml-64' : 'ml-0'}`}>
        ...
        <main className="flex-1 space-y-6 p-4 md:p-6 max-w-7xl mx-auto w-full pb-24 lg:pb-6">
            ...
            <div className="space-y-6">
              ...
              <div className="hidden md:flex flex-row items-center justify-between gap-3">
                ...
                <button
                  ...
                  className={`px-6 py-2.5 rounded-xl text-xs font-extrabold flex items-center justify-center gap-2 shadow-md shrink-0 transition-transform ${
                    isEntryLocked
              ...
              <div className="flex md:hidden flex-col gap-2.5">
                {/* Row 1: Search Bar (slightly smaller flex-1) + Entry Button */}
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={isEntryLocked}
                    className={`px-3.5 py-2 rounded-xl text-xs font-extrabold flex items-center justify-center gap-1.5 shadow-sm shrink-0 ${
                      isEntryLocked
                        ? 'neu-pressed text-[var(--color-text-tertiary)] opacity-60 cursor-not-allowed'
                        : 'neu-btn-primary text-white cursor-pointer'
                    }`}
                  >
                    {isEntryLocked ? <Lock className="h-3.5 w-3.5 text-red-500" /> : <Plus className="h-3.5 w-3.5" />}
                    <span>Entry</span>
                  </button>
                </div>
          ...
          {activeTab === 'files' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  {/* Grid View & List View Mode Toggle Button (Desktop & Mobile) */}
                ...
                <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
                  {files.map((file) => (
                    <div key={file.id} className="rounded-2xl neu-card p-4 relative group">
                      {/* Top Right Action Options: Star & Delete */}
          ...
          {/* TAB 3: STARRED (Available to ALL Users) */}
          {activeTab === 'starred' && (
            <div className="space-y-6">
              <div>
              ...
              {starredEntries.length === 0 && starredFiles.length === 0 ? (
                <EmptyState icon={Star} title="No starred items in TripO Lead" description="Star hotel entries or files for quick access." />
              ) : (
                <div className="space-y-6">
      ...
      {/* Dedicated Mobile Bottom Navigation Bar for TripO Lead */}
      <TripoLeadMobileBottomNav activeTab={activeTab} onTabChange={setActiveTab} />
