...
import type { FreelanceLeadTab } from './FreelanceLeadSidebar';

interface FreelanceLeadMobileBottomNavProps {
  activeTab: FreelanceLeadTab;
  onTabChange: (tab: FreelanceLeadTab) => void;
}

const navItems: { id: FreelanceLeadTab; label: string; icon: any; color: string; activeText: string; activeBg: string }[] = [
  {
    id: 'home',
    label: 'Home',
    icon: Home,
...
export function FreelanceLeadMobileBottomNav({ activeTab, onTabChange }: FreelanceLeadMobileBottomNavProps) {
  return (
    <nav
      ...
      aria-label="Freelance Lead Mobile Bottom Navigation"
    >
      {navItems.map((item) => {
