...
import { ProofSidebar, type ProofTab } from './ProofSidebar';
import { ProofMobileBottomNav } from './ProofMobileBottomNav';
import { ProofCreateFolderModal } from './ProofCreateFolderModal';
...
export function ProofView({ task }: ProofViewProps) {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const [activeTab, setActiveTab] = useState<ProofTab>('home');
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');

  // Folder states for Proof Home
  const [createFolderModalOpen, setCreateFolderModalOpen] = useState(false);
  const [openedFolder, setOpenedFolder] = useState<ProofFolder | null>(null);
  ...
  // Query Proof section folders
  const { data: proofFolders = [], isLoading: loadingFolders } = useQuery({
    queryKey: ['proofFolders', task.id, searchQuery],
    queryFn: () => getProofFolders(task.id, searchQuery),
  });

  // Query files inside opened Proof folder
  const { data: folderFiles = [], isLoading: loadingFolderFiles } = useQuery({
    queryKey: ['proofFolderFiles', task.id, openedFolder?.id, searchQuery],
    queryFn: () => getFiles(user?.id || '', openedFolder?.id || null, { search: searchQuery }),
    enabled: !!openedFolder,
  });

  // Query task level files & trash
  const { data: files = [] } = useQuery({
    queryKey: ['proofFiles', task.id],
    queryFn: () => getTaskFiles(task.id),
  });

  const { data: trashFiles = [] } = useQuery({
    queryKey: ['proofTrashFiles', task.id],
    queryFn: () => getTaskTrashFiles(task.id),
  });

  // Create Folder Mutation
  const createFolderMutation = useMutation({
    mutationFn: (folderName: string) => createProofFolder(task.id, folderName, user?.id),
    onSuccess: (newFolder) => {
      queryClient.invalidateQueries({ queryKey: ['proofFolders', task.id] });
  ...
  const deleteFolderMutation = useMutation({
    mutationFn: (folderId: string) => deleteProofFolder(task.id, folderId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['proofFolders', task.id] });
  ...
  const toggleStarFileMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['proofFiles', task.id] });
      queryClient.invalidateQueries({ queryKey: ['proofFolderFiles', task.id, openedFolder?.id] });
  ...
  const softDeleteFileMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['proofFiles', task.id] });
      queryClient.invalidateQueries({ queryKey: ['proofFolderFiles', task.id, openedFolder?.id] });
      queryClient.invalidateQueries({ queryKey: ['proofTrashFiles', task.id] });
  ...
  return (
    <div className="flex min-h-screen bg-[var(--color-surface-secondary)] relative">
      {/* Proof Dedicated Desktop Sidebar */}
      <ProofSidebar
        isOpen={sidebarOpen}
        ...
        <main className="flex-1 space-y-6 p-4 md:p-6 max-w-7xl mx-auto w-full pb-24 lg:pb-6">
          ...
          {activeTab === 'home' && (
            <div className="space-y-6">
              {/* IF NO FOLDER IS OPENED: SHOW PROOF FOLDERS & SEARCH/CREATE BAR */}
              {!openedFolder ? (
                <div className="space-y-6">
                  {/* Top Action Bar: Search Bar & Create Button */}
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                    ...
                    <div className="relative flex-1 min-w-[200px]">
                      ...
                      <input
                        ...
                        placeholder="Search Proof folders..."
                    ...
                    <button
                      ...
                      className="neu-btn-primary px-5 py-2.5 rounded-xl text-xs font-extrabold text-white flex items-center justify-center gap-2 cursor-pointer shadow-md shrink-0 hover:scale-[1.02] transition-transform"
                    >
                  ...
                  ) : proofFolders.length === 0 ? (
                    <EmptyState
                      icon={FolderOpen}
                      title={searchQuery ? 'No matching Proof folders' : 'No Proof folders created yet'}
                      description={
                        searchQuery
                          ? 'Try adjusting your search query.'
                          : 'Click "Create" to organize your Proof files into folders.'
                      }
                    />
                  ) : (
                    <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
                      {proofFolders.map((folder) => (
                        <div
                          key={folder.id}
                        ...
                        >
                          <button
                            ...
                            className="absolute top-3 right-3 z-10 opacity-0 group-hover:opacity-100 transition-opacity text-[var(--color-text-tertiary)] hover:text-red-500 p-1 cursor-pointer"
                            title="Delete Folder"
                          >
                          ...
                          <div className="flex flex-col items-center text-center space-y-3 py-2">
                            <div className="h-14 w-14 rounded-2xl neu-circle flex items-center justify-center text-blue-500 bg-blue-500/10">
                              <Folder className="h-7 w-7 fill-blue-500/20" />
                            </div>
                ...
                /* IF A FOLDER IS OPENED: SHOW FOLDER DETAILS & FILE LIST */
                <div className="space-y-6">
          ...
          {activeTab === 'files' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <h1 className="hidden md:block text-2xl font-black text-[var(--color-text-primary)]">Proof Files</h1>
                  <p className="hidden md:block text-xs font-semibold text-[var(--color-text-secondary)] mt-1">Files stored under Proof</p>
              ...
              {files.length === 0 ? (
                <EmptyState icon={FolderOpen} title="No files in Proof" description="Upload a file to Proof to get started." />
              ) : filesViewMode === 'grid' ? (
                <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
                  {files.map((file) => (
                    <div key={file.id} className="rounded-2xl neu-card p-4 relative group">
          ...
          {activeTab === 'recent' && (
            <div className="space-y-6">
              <div>
                <h1 className="hidden md:block text-2xl font-black text-[var(--color-text-primary)]">Proof Recent</h1>
                <p className="hidden md:block text-xs font-semibold text-[var(--color-text-secondary)] mt-1">Recent Proof activity</p>
                <h1 className="block md:hidden text-xl font-black text-[var(--color-text-primary)]">Recent</h1>
              </div>
              <EmptyState icon={Calendar} title="No recent activity" description="Proof files and folders will appear here as updated." />
            </div>
          )}
          ...
          {activeTab === 'trash' && (
            <div className="space-y-6">
              <div>
                <h1 className="hidden md:block text-2xl font-black text-[var(--color-text-primary)]">Proof Trash</h1>
                <p className="hidden md:block text-xs font-semibold text-[var(--color-text-secondary)] mt-1">Deleted items in Proof</p>
                <h1 className="block md:hidden text-xl font-black text-[var(--color-text-primary)]">Trash</h1>
              </div>

              {trashFiles.length === 0 ? (
                <EmptyState icon={Trash2} title="Trash is empty" description="Deleted items in Proof will appear here." />
              ) : (
                <div className="neu-card rounded-2xl overflow-hidden border border-[var(--color-border-light)]/40">
                  ...
                  <div className="divide-y divide-[var(--color-border-light)]/40">
                    {trashFiles.map((file) => (
                      <div key={file.id} className="p-4 flex items-center justify-between gap-3">
                        ...
                        <button
                          onClick={() => restoreFile(file.id).then(() => queryClient.invalidateQueries({ queryKey: ['proofTrashFiles', task.id] }))}
      ...
      {/* Proof Mobile Bottom Navigation */}
      <ProofMobileBottomNav
        activeTab={activeTab}
        onTabChange={(tab) => {
          setActiveTab(tab);
          setOpenedFolder(null);
